...
class MeshtasticManager(QObject, threading.Thread):
    ...
    def __init__(self, dev_path=None):
        ...
        self._data.acknowledgment = type('', (), {})()  # Create an empty object to hold acknowledgment flags
    ...
    def store_or_update_node(self, node:MeshtasticNode) -> None:
        ...
        if not str(node.id) in nodes.keys():
            ...
            def __get_nodes_fields():
                return [field for field in fields(MeshtasticNode) if not field.name.startswith('_')]

            tmp = nodes[str(node.id)].firstseen
            node.firstseen = tmp
            for f in __get_nodes_fields():
                if getattr(nodes[str(node.id)],f.name) != getattr(node, f.name):
                    if getattr(node, f.name) is not None:
                        setattr(nodes[str(node.id)], f.name, getattr(node, f.name))
    ...
    def connect_device(self) -> bool:
        if self._config.interface is not None:
            return
        try:
            self._config.interface = meshtastic.serial_interface.SerialInterface(devPath=self._config.device_path)
    ...
    def retrieve_local_node_configuration(self) -> None:
        ...
        self.notify_frontend(MessageLevel.INFO, "Local node configuration retrieved.")
    ...
    def on_ack(self, response, event):
        ...
        key = list(filter(lambda x:messages_list[x].mid == response["decoded"]["requestId"], messages_list.keys()))
        if len(key) == 0:
            return
        key = key[0]

        messages_list[key].date=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if "rxRssi" in response: messages_list[key].rx_rssi=response['rxRssi']
        if "rxSnr" in response: messages_list[key].rx_snr=response['rxSnr']
        if "rxhopLimit" in response: messages_list[key].hop_limit=response['hopLimit']
        if "hopStart" in response: messages_list[key].hop_start=response['hopStart']
        if "wantAck" in response: messages_list[key].want_ack=response['wantAck']
                ...
                if 'payload' in decoded and isinstance(decoded['payload'], bytes):
                    try:
                        ...
                        if data.startswith(ANNOUNCE_IDENTIFIER):
                            ...
                            self._data.received_chunks[file_name] = [None] * total_chunks
                            message = f"File announcement received: {file_name}, Size: {file_size} bytes, Total Chunks: {total_chunks}"
                            print(Fore.BLUE + message)
                            self.notify_data(message=message, message_type="INFO")
                            ...
                            if len(parts) == 4:
                                ...
                                if self._data.received_chunks[file_name][chunk_index] is None:
                                    ...
                                    if all(self._data.received_chunks[file_name]):
                                        complete_data = b''.join(self._data.received_chunks[file_name])
                                        self.save_file(file_name, complete_data)
                        else:
                            current_message = data.decode('utf-8').strip()
                            if len(current_message) > 0:
                                messages_list = self._data.get_messages()
                                key = list(filter(lambda x:messages_list[x].mid == packet["id"], messages_list.keys()))
                                if len(key) == 0:
                                    ...
                                    print(Fore.GREEN + f"Received message: {m}")
                        ...
                        self.notify_data(f"Received non-text payload: {decoded['payload']}", message_type="INFO")
                        self.store_received_packet(f"Received non-text payload: {decoded['payload']}")

                else:
                    print(Fore.LIGHTBLACK_EX + f"Received packet without text payload: {decoded}")
                    self.notify_data(f"Received packet without text payload: {decoded}", message_type="INFO")
                    self.store_received_packet(f"Received packet without text payload: {decoded}")

            else:
                print(Fore.LIGHTBLACK_EX + f"Received packet without 'decoded' field: {packet}")
                self.notify_data(f"Received packet without 'decoded' field: {packet}", message_type="INFO")
                self.store_received_packet(f"Received packet without 'decoded' field: {packet}")
    ...
    def request_missing_chunks(self, file_name):
        ...
        missing_chunks = [i for i, chunk in enumerate(self._data.received_chunks[file_name]) if chunk is None]
        if missing_chunks:
            request_message = f"REQ:{file_name}:{','.join(map(str, missing_chunks))}"
            self._config.interface.sendText(request_message, self._config.destination_id)
    ...
    def send_text_message(self, message:MeshtasticMessage):
        ...
        self.notify_frontend(MessageLevel.INFO, trace)
        self.notify_message()
        ack_event.wait(timeout=self._config.get_timeout())  # Wait for acknowledgment or timeout after the set period
    ...
    def send_data_in_chunks(self, data, file_name, progress_callback: Optional[Callable[[int, int], None]] = None, channel_index=0):
        ...
        for i in range(total_chunks):
            ...
            if retries == self._config.get_retransmission_limit():
                print(Fore.RED + f"Failed to send chunk {i+1}/{total_chunks} after {self._config.retransmission_limit} attempts. Aborting.")
    ...
    def update_node_info(self, packet) -> None:
        if packet["decoded"]["portnum"] == "TELEMETRY_APP":
            n = MeshtasticNode(
                id=self._nodeNumToId(packet["from"]),
                lastseen=datetime.datetime.fromtimestamp(packet["decoded"]["telemetry"]["time"]).strftime('%Y-%m-%d %H:%M:%S') if "lastHeard" in packet and packet["lastHeard"] is not None else None,
                batterylevel=packet["decoded"]["telemetry"]["deviceMetrics"]["batteryLevel"],
                txairutil=str(round(packet["decoded"]["telemetry"]["deviceMetrics"]["airUtilTx"], 2)) if "deviceMetrics" in packet and "airUtilTx" in packet["deviceMetrics"] else None,
            )
    ...
    def retrieve_nodes(self, include_self: bool=True) -> list:
        ...
        if self._config.interface.nodesByNum:
            logging.debug(f"self._config.interface.nodes:{self._config.interface.nodes}")
    ...
    def add_channel(self, name):
        ...
        try:
            ...
            disabled_channel.settings.psk = self._config.interface.localNode.channels[0].settings.psk  # Using the same PSK as primary for simplicity
            self._config.interface.localNode.writeChannel(disabled_channel.index)
        except Exception as e:
    ...
    def sendTraceRoute(self, dest: Union[int, str], hopLimit: int, channelIndex: int=0):
        ...
        waitFactor = min(len(self._config.interface.nodes) - 1 if self._config.interface.nodes else 0, hopLimit)
        self.notify_frontend(MessageLevel.INFO, f"Traceoute started to {dest}.")
